...
class SplitNodeModelTest {
    ...
    @BeforeEach
    fun setUp() {
        nodeModel = SplitNodeModel(jacksonObjectMapper())
    ...
    fun `test node metadata is properly configured`() {
        ...
        assertEquals("org.projectcontinuum.feature.base.analytics.node.SplitNodeModel", metadata.id)
    ...
    fun `test with empty input stream`() {
        ...
        verify(mockPortWriter1,
          never()
        ).write(any(), any())
        verify(mockPortWriter2,
          never()
        ).write(any(), any())
    }
